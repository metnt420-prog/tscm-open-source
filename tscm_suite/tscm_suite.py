...
class BladeRFCLIBridge:
    ...
    EXPECTED_BYTES = 8388608  # n=2097152 total samples * 2(I/Q) * 2bytes
            ...
            while self.active:
                try:
                    ...
                    cmd_script = (f"set frequency rx {self.freq}\n"
                                ...
                                f"set biastee rx1 on\n"
                                f"set biastee rx2 on\n"
                                f"rx config file={tmpname} format=bin n=2097152\n"
                                f"rx start\nrx wait 4000\n")
                    with open(scriptfile, 'w') as f:
                        f.write(cmd_script)
        ...
        while self.running:
            ...
            if bladerf_active:
                ...
                try:
                    ...
                    self.log.info(f'RF-AM(BR) check: bfreq={actual_bfreq} iq1={len(iq1) if iq1 is not None else -1} cyc={self.cycle_count}')
                    if actual_bfreq is not None and abs(actual_bfreq - 2450e6) < 10e6 and iq1 is not None and len(iq1) >= 4096:
                        ...
                        if not hasattr(self, '_rf_baseband_buf'):
                            self._rf_baseband_buf = deque(maxlen=48000*300)
                        ...
                        if rf_rms > 0.0001:
                            ...
                            if len(rf_audio_nb) >= 100:
                                import wave; import os as _os; import time as _time2
                                ts = time.strftime('%Y%m%d_%H%M%S')
                            ...
                            if buf_len >= 48000 * 5:  # 5 seconds - accumulate across passes
                                ts = time.strftime('%Y%m%d_%H%M%S')
                                out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'rf_baseband')
                                os.makedirs(out_dir, exist_ok=True)

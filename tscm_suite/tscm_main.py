    ...
    def resolve_sources(self, current_lat, current_lon):
        ...
        for fp, obs_list in self.observations.items():
            ...
            if is_confirmed:
                ...
                if prev and prev.get('lat'):
                    ...
                    if jump_m > 500:
                        ...
                        self.sources[fp] = {'lat': prev['lat'], 'lon': prev['lon'],
                ...
                self.sources[fp] = {'lat': src_lat, 'lon': src_lon,
                    'classification': classification, 'first_seen': obs[0]['ts'],
                    'last_seen': now, 'freq': freq, 'detector': detector,
                    'range': measured_rng, 'snr': latest.get('snr', 0),
                    'bearing': latest.get('bearing', 0),
            ...
            if latest['bearing'] == 0.0:
                ...
                # Bearing without any range estimate: place at 500m along bearing line
                bearings = [o['bearing'] for o in obs if o.get('bearing', 0) != 0.0]
                avg_bearing = float(np.mean(bearings)) if bearings else latest['bearing']
                src_lat, src_lon = self._bearing_to_xy(
                    obs[-1]['lat'], obs[-1]['lon'], avg_bearing, 500)  # 500m default (was 1km)
                self.sources[fp] = {'lat': src_lat, 'lon': src_lon, 'bearing': avg_bearing,
                    'classification': classification, 'first_seen': obs[0]['ts'],
                    'last_seen': now, 'freq': freq, 'detector': detector,
                    'method': 'bearing_500m_est', 'observations': len(obs),
                    'bearing_samples': len(bearings),
                    'triangulated': False,
                    'confidence': 'low'}
...
if(dets.length>0){h+='<div style="color:#888;font-weight:bold;border-top:1px solid #333;padding-top:4px;margin-top:4px">DETECTIONS ('+dets.length+')</div>';
...
class MapHandler(BaseHTTPRequestHandler):
    ...
    def _build_signal_chain(self, raw=False):
        ...
        if raw:
            ...
            # === FILTERED MODE: show bearing+range sources with dedup ===
            # Triangulated sources get solid pins. Bearing+range sources get
            # lighter pins. All merged within 15 degrees in the dedup pass below.
            for s in sources:
                det = s.get('detector', 'unknown')
                bearing = s.get('bearing')
                freq = s.get('freq', 0)
                ...
                # Skip at_observer sources with no bearing (list-only)
                ...
                # Triangulated sources have their own lat/lon from bearing intersection
                src_lat = s.get('lat', obs_lat)
                src_lon = s.get('lon', obs_lon)
                if src_lat == 0 and src_lon == 0:
                    src_lat, src_lon = obs_lat, obs_lon
                if bearing and abs(bearing) > 0.5 and not dist:
                    lat2, lon2 = brg2ll(obs_lat, obs_lon, bearing, dist)
                else:
                    lat2, lon2 = src_lat, src_lon
            ...
            # === SECOND PASS: non-triangulated sources → list-only (no map element) ===
            for s in sources:
                if s.get('triangulated'):
                    continue  # already placed on map above
                det = s.get('detector', 'unknown')
                ...
                classification = s.get('classification', '')
                method = s.get('method', '?')
                threat_short = _clean(det[:12].upper())

                markers.append({'lat': 0, 'lon': 0, 'type': 'detection',
                    'detector': _clean(det),
                    'bearing': round(bearing, 1) if bearing else 0,
                    'freq': freq_str, 'observations': obs_count,
                    'classification': _clean(classification),
                    'color': '#444', 'threat': threat_short,
                    'method': method})
...
class TSCMSystem:
    ...
    def run(self):
        ...
        while self.running:
            cycle_start=time.time(); self.cycle_count+=1

"""
Server-side map renderer with Esri satellite tiles (no API key).
...
def _download_tile(z, x, y, timeout=5):
    ...
    for attempt in range(3):
        ...
        _tile_server_idx += 1
        # Esri ArcGIS: /tile/{z}/{y}/{x}  |  OSM fallback: /{z}/{x}/{y}.png
        if 'arcgisonline' in server:
            url = '%s/%d/%d/%d' % (server, z, y, x)
        else:
            url = '%s/%d/%d/%d.png' % (server, z, x, y)
        try:
            ...
            if len(data) > 500:  # satellite tiles are larger than blank placeholder
...
def _clamp_bearing_to_map(cx, cy, bearing_deg, map_w, map_top, map_bot, scale):
    """Clamp a bearing line to the visible map edge. Returns (px, py) at edge."""
...
def render_map(detections_data, width=3200, height=1800, zoom=18):
    ...
    # Auto-zoom: compute required zoom to fit all markers within map bounds
    if sources:
        marker_lats = [s.get('lat') for s in sources if s.get('lat') is not None and abs(s.get('lat', 0)) > 0.001]
        marker_lons = [s.get('lon') for s in sources if s.get('lon') is not None and abs(s.get('lon', 0)) > 0.001]
        if marker_lats and marker_lons:
            lat_min = min(min(marker_lats), observed_lat)
            lat_max = max(max(marker_lats), observed_lat)
            lon_min = min(min(marker_lons), observed_lon)
            lon_max = max(max(marker_lons), observed_lon)
            # Required zoom: map area is 85% of width x (height - 20)
            map_area_w = width * 0.85
            map_area_h = height - 20
            # Each tile is 256px. At zoom Z, world is 256*2^Z px wide (360deg)
            # So meters_per_pixel = 360 / (256 * 2^Z) * 111319 * cos(lat)
            # Required zoom: lat_span * 111319 / map_area_h < meters_per_pixel
            lat_span_m = (lat_max - lat_min) * 111319.0
            lon_span_m = (lon_max - lon_min) * 111319.0 * math.cos(math.radians((lat_min + lat_max) / 2))
            # Need at least this many meters per pixel
            required_mpp_lat = lat_span_m / (map_area_h * 0.85)  # 85% fill
            required_mpp_lon = lon_span_m / (map_area_w * 0.85)
    ...
    use_fast_path = len(sources) > FAST_PATH_THRESHOLD
    ...
    # Progressive tile loading: try cache first, batch-fetch missing
    observed_lat = observer.get('lat', OBS_LAT)
    observed_lon = observer.get('lon', OBS_LON)
    ...
    # Phase 1: composite cached tiles instantly (no network)
    for tx in range(tx_min, tx_max + 1):
        for ty in range(ty_min, ty_max + 1):
            tile_data = _get_cached_tile(zoom, tx, ty)
            if tile_data:
                try:
    ...
    # Phase 2: Download missing tiles (with timeout short enough to avoid blocking)
    import concurrent.futures as _cf
    def _fetch_one(tx, ty):
        return (tx, ty, _download_tile(zoom, tx, ty, timeout=3))

    missing = [(tx, ty) for tx in range(tx_min, tx_max + 1)
                       for ty in range(ty_min, ty_max + 1)
                       if not _get_cached_tile(zoom, tx, ty)]

    if missing:
        with _cf.ThreadPoolExecutor(max_workers=4) as _ex:
            _futures = [_ex.submit(_fetch_one, tx, ty) for tx, ty in missing]
            for _f in _cf.as_completed(_futures, timeout=8):
                try:
                    tx, ty, data = _f.result()
                    if data:
                        tm = Image.open(io.BytesIO(data))
                        ox = int(tx * TS - (obs_px - map_w/2))
                        oy = int(ty * TS - (obs_py - (map_top + map_bot)/2))
                        img.paste(tm, (ox, oy))
                        got += 1
    ...
    # Range rings - scale-adaptive: zoom 18+ gets sub-100m resolution
    ring_distances = [10, 25, 50, 100, 200, 500] if zoom >= 18 else [50, 100, 200, 500, 1000, 2000]
    for dist_m in ring_distances:
        r = int(dist_m * scale)
        if 5 < r < map_w//2:
    ...
    if use_fast_path:
        # FAST PATH: bulk stats + skip individual per-point draws for speed
        no_bearing = {}
        for s in sources:
            det = s.get('detector', 'unknown')
            if det not in no_bearing:
                no_bearing[det] = {'count': 0, 'color': DETECTOR_COLORS.get(det, (11, 191, 255)),
                                   'freqs': [], 'class': s.get('classification', '')}
            no_bearing[det]['count'] += s.get('observations', 1)
            freq = s.get('freq', 0)
            if freq > 0:
        ...
        # STANDARD PATH: full marker rendering for <500 sources
        for s in sources:
            det = s.get('detector', 'unknown')
            freq = s.get('freq', 0)
            ...
            if triangulated and bearing is not None and abs(bearing) > 0.5:
                ...
                sx_edge, sy_edge = _clamp_bearing_to_map(cx, cy, bearing, map_w, map_top, map_bot, scale)
                draw.line([(cx, cy), (sx_edge, sy_edge)], fill=(100, 100, 100), width=1)
                # Endpoint marker at edge
                edr = 5
                draw.ellipse([sx_edge-edr, sy_edge-edr, sx_edge+edr, sy_edge+edr], fill=None, outline=(100, 100, 100), width=2)
                ...
                # Endpoint marker at edge
                edr = 5
                draw.ellipse([sx_edge-edr, sy_edge-edr, sx_edge+edr, sy_edge+edr], fill=None, outline=(200, 200, 0), width=2)

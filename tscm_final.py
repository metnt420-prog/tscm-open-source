...
Based on user's reference script, added:
  - SourceLocalizationEngine: AoA + passive-radar â†’ real source positions
...
Physics:
  MW voice attack â†’ hits victim body â†’ carbon interaction â†’ ultrasound emission FROM victim
  BladeRF MIMO AoA â†’ bearing to RF source (transmitter or victim re-radiation)
  Cross-correlating two RX channels â†’ passive radar delay â†’ range to reflector
  When observer moves (GPS), bearing lines from different positions intersect â†’ triangulated source position
...
import numpy as np  # global numpy import â€” used throughout
from collections import deque, defaultdict
from http.server import HTTPServer, BaseHTTPRequestHandler
from tscm_parametric_countermeasures import (
    UltrasonicBeatFrequencyDetector,
    UltrasonicNoiseJammer,
    CrossLayerParametricCorrelator,
)
...
import warnings
from propagation_modes import get_propagation_classifier
...
# ===================== CONFIGURATION =====================
class Config:
    # GPS - u-blox on COM4 (ZED-F9P on COM5 if available)
    # Anti-spoof: cross-check against HOME, reject jumps >500m
    # WiFi geolocation disabled (spoofable via C2 WiFi bridge)
    GPS_PORT = 'COM4'    # u-blox GPS primary (confirmed)
    GPS_PORT_2 = 'COM5'  # ZED-F9P fallback
    ...
    ANTENNA_SPACING = 0.0625  # Î»/2 at 2.4 GHz
    MAP_THREAT_LABELS = {
        'carbon_interaction': 'SPOOFING: MWâ†’bodyâ†’carbon re-emission (YOU are the source, not attacker)',
    ...
    BEARING_OFFSET = 0  # ZERO BIAS - raw measurements only
    # Array axis: which direction does the line from rx1â†’rx2 point?
    # 0 = north, 90 = east, 180 = south, 270 = west
    ARRAY_AXIS_DEGREES = 0  # ZERO BIAS - raw array orientation
    ...
    HACKRF_OFFSET_M = 0         # ZERO BIAS - co-located with BladeRF

    # RTL-SDR (Realtek RTL2838U) â€” third sensor for triangulation
    RTLSDR_ENABLED = True
    RTLSDR_FREQ = 850e6        # 850 MHz (cellular/ISM â€” different from BladeRF 2.4G and HackRF 450M)
    ...
    RTLSDR_VID = '0BDA'
    RTLSDR_PID = '2838'
    # WiFi dongle (Realtek 8812AU) â€” used for WiFi AP scanning / C2 detection
    WIFI_DONGLE_VID = '0BDA'
    WIFI_DONGLE_PID = '8812'

    # Source localization
    TRIANGULATION_MIN_OBS = 1  # lowered for faster positioning
    TRIANGULATION_MAX_AGE = 300
    BEARING_LINE_LENGTH = 500
    MIN_OBS_FOR_MAP = 1  # show all sources with any data
    MIN_OBS_FOR_ESTIMATE = 2  # lowered - ultrasound needs faster positioning
...
class WiggleOrchestrator:
    ...
    Six-axis wiggle doctrine:
      1. Cycle Jitter     â€” random Â±30% inter-cycle delay
      2. Sweep Shuffle    â€” randomize equal-priority band order
      3. Spatial Dither   â€” synthetic Â±5m position drift (Perlin-like smooth)
      4. Amplitude Wiggle â€” Â±3dB + Â±5Â° on null-steering TX bursts
      5. Bearing Dither   â€” Â±0.5-3Â° Gaussian noise on published bearings
      6. Temporal Dither  â€” Â±15-25% jitter on all periodic operations
    ...
    def __init__(self, master_seed=None):
        ...
        # Independent child RNGs â€” compromising one does not reveal others
        ...
        self.cycle_jitter_pct = 0.30        # Â±30% cycle jitter
        self.sweep_shuffle_threshold = 0.10  # shuffle bands within 10% dwell
        self.spatial_dither_m = 5.0          # Â±5m when GPS-lost
        self.spatial_dither_m_gps = 1.0      # Â±1m when GPS-valid
        self.amplitude_jitter_db = 3.0       # Â±3dB on null steering
        self.phase_jitter_deg = 5.0          # Â±5Â° on null steering
        self.bearing_dither_mimo = 0.5       # Â±0.5Â° for BladeRF MIMO AoA
        self.bearing_dither_hackrf = 2.0     # Â±2Â° for HackRF ferrite
        self.bearing_dither_acoustic = 3.0   # Â±3Â° for Intel array
        self.temporal_jitter_pct = 0.20      # Â±20% on periodic ops
    ...
    # â€”â€” Cycle Jitter â€”â€”
    def next_cycle_delay(self, nominal_s=0.2):
        """Return jittered cycle delay in seconds (Â±30%)."""
    ...
    # â€”â€” Amplitude/Phase Wiggle for Null Steering â€”â€”
    ...
    # â€”â€” Temporal Dither â€”â€”
    def jitter_interval(self, nominal_seconds):
        """Return jittered interval for any periodic operation (Â±20%)."""
    ...
    # USB watchdog
    BLADERF_VID = "2cf0"
    BLADERF_PID = "5250"  # bladeRF 2.0 micro xA9 (was 5251 â€” wrong PID, watchdog never matched)
...
class CourtLogger:
    ...
    - SHA256 chain link (prev_hash + current entry â†’ current hash)
...
class RSSIPathLossRanger:
    ...
    def estimate_range(self, rms, freq, sdr_type='bladerf', tx_power_dbm=None):
        ...
        if snr_linear < 1.5:  # barely above noise â€” no range estimate
            return None, 0
...
class SourceLocalizationEngine:
    ...
    Classifies sources as TRANSMITTER (RF origin) or VICTIM (carbon MW interaction â†’ ultrasound).
    ...
    def __init__(self, log):
        ...
        # Propagation mode classifier for multi-mode triangulation
        self.prop_mode = get_propagation_classifier(log=self.log)
    ...
    def add_observation(self, fingerprint, obs_lat, obs_lon, bearing_deg,
        ...
        # === INPUT VALIDATION â€” prevent spoofed/injected data from creating phantom sources ===
        ...
        if _heuristic_only:
            ...
            #    BladeRF MIMO AoA is measured at 2.4 GHz. Only inject for sources
            #    actually in the SDR band (>= 1 MHz). Low-frequency detectors
            #    (acoustic, ultrasound, EEG) fall through to acoustic AoA or no bearing.
            if not _aoa_injected and self.current_aoa != 0.0:
                _is_rf_source = freq >= 1e6  # Only MHz+ SDR-band sources get BladeRF AoA
        ...
        # Re-enabled with massively scaled range factor to prevent
        # artificial 30-500m clustering. Ferrite computes ~31km at 570MHz.
        ...
        # physical position â†’ two bearing lines from two positions = true triangulation
    ...
    def _compute_position_confidence(self, bearing_samples, range_m, num_obs):
        ...
        if range_m and 30 < range_m < 2000:
            ...
            range_conf = 0.3  # near-field â€” inaccurate
        elif range_m and range_m >= 2000:
            range_conf = 0.4  # far-field â€” less accurate
        else:
            range_conf = 0.5
    ...
    def resolve_sources(self, current_lat, current_lon):
        ...
        for fp, obs_list in self.observations.items():
            ...
            # --- Propagate mode classification from observation history ---
            # Use the propagation mode classifier to determine HOW this signal
            # is traveling: LOS, OTH/skywave, satellite relay, multipath, ground wave, etc.
            # This affects how we interpret the bearing for position estimation.
            _bearing_vals = [o.get('bearing', 0) for o in obs if o.get('bearing', 0) != 0.0 and not o.get('no_bearing', True)]
            _bearing_stability = float(np.std(_bearing_vals)) if len(_bearing_vals) >= 3 else None
            _mode_info = self.prop_mode.classify(
                fp, freq, latest.get('bearing', 0), latest.get('snr', 0),
                detector, obs, bearing_stability=_bearing_stability,
                has_multiple_bearings=False,  # checked in multipath pass below
                triangulated=False
            )
            _prop_mode = _mode_info['mode']
            _prop_conf = _mode_info['confidence']
            _prop_params = _mode_info['params']
            # If mode is unknown and freq > 30 MHz, default to LOS
            if _prop_mode == 'unknown' and freq > 30e6:
                _prop_mode = 'los'
                _prop_conf = 0.5
            ...
            # DISABLED: Single-point bearing+range estimates using shared
            # HackRF range injection produce artificial distance compression
            # (all sources at 30-500m). Only true multi-position triangulation
            # or cross-sensor bearing intersection produces real positions.
            #
            # has_real_bearing restored - sources NEED bearing_range_est to appear.
            # Range scale increased to prevent compression.
            ...
            has_real_freq = latest.get('freq', 0) > 0  # was: >1e6 â€” blocked ultrasound/person
            ...
            if is_confirmed:
                ...
                if prev and prev.get('lat'):
                    ...
                    if jump_m > 500:
                        # Spoofed bearing or bogus range â€“ keep at last known position
            ...
            # This is the TRUE triangulation path: bearing lines from â‰¥3 distinct
            # observer positions (>5m apart) intersect â†’ source position.
            ...
            is_sdr_band = freq > 0  # was: >1e6 â€” now accepts ALL freq bands (ultrasound, radar bodies, MW)
            if is_sdr_band and len(obs) >= Config.TRIANGULATION_MIN_OBS:
                # TRUE triangulation: bearing lines from â‰¥2 distinct observer
                # positions (>5m apart) intersect â†’ source position.
                ...
                if len(bearing_obs) >= Config.TRIANGULATION_MIN_OBS:
                    ...
                    if len(distinct) >= 2:
                        # Minimum bearing spread: reject if all bearings within Â±15Â°
                        bearings_span = [o['bearing'] for o in distinct]
                        if max(bearings_span) - min(bearings_span) < 30.0:
                            pass  # Bearings too parallel â€” skip triangulation
                        else:
                            intersections = []
        ...
        # different frequencies â†’ different fingerprints â†’ never combine in the
        ...
        if self.hackrf_lat is not None and self.hackrf_lon is not None:
            ...
            if bladerf_bearings and hackrf_bearings:
                ...
                # Check positions are actually distinct (>3m apart â€” fixed sensors ~5m apart)
                ...
                if math.sqrt(dx*dx + dy*dy) > 3.0:  # 3m min â€” fixed sensors are ~5m apart
                    intersections = []
                    for b_obs in bladerf_bearings[-8:]:
                        for h_obs in hackrf_bearings[-8:]:
                            # Frequency correlation guard: DISABLED for broadband emitters.
                            # The adversary transmits across ALL bands simultaneously â€” GHz microwave
                    ...
                    if intersections:
                        ...
                        # Check convergence â€” intersections should cluster within ~500m
        ...
        # Three bearing lines from three positions â†’ real intersection even without GPS movement.
...
def superhet_demod(signal, fs, lo_freq, bandwidth=3000):
    ...
    1. Mix signal with local oscillator at lo_freq â†’ produces IF (difference frequency)
    2. Low-pass filter to isolate the AM envelope (baseband)
    3. Envelope detect the result â†’ voice audio
...
class ForcedThoughtDetector:
    """
    RF envelope â†’ audio cross-correlation detector (primary).
    ...
    def detect(self):
        ...
        # Cross-correlation with lag - accounts for RFâ†’audio latency
        if len(env_ds) >= 400:
...
class SilentSoundDetector:
    ...
    Fed by laptop mic (48kHz) â€” scans 15-24kHz ultrasonic range
    ...
    def detect(self, iq=None, fs=None):
        ...
        for p in peaks[:10]:
            ...
            bw_half = int(4000 / (self.fs / n))  # Â±4kHz in FFT bins
...
class GhostHunterSNN:
    ...
    def detect(self):
        ...
        if np.all(feat_range < 1e-6):
            return []  # sensor producing constant output â€” not real data
        ...
        if len(active_dims) < 2:
            return []  # fewer than 2 active dimensions â€” noise level
        outliers=np.any(np.abs(feats-mean)>4*std,axis=1)  # 4Ïƒ (was 3)
        outlier_count = int(np.sum(outliers))
        # Require outlier burst density: â‰¥5 outliers AND >20% of recent samples
...
class FingerprintingDetector:
    ...
    def detect(self):
        ...
        # Threshold tightened: require â‰¥5 peaks above 6x noise floor AND peak-spread > 3% of BW
        ...
        if not features or 'center_freq' not in features:
            ...
            if freq_spread < 0.5e6: return []  # peaks too narrow â€” likely a single carrier, not a fingerprintable signal
        ...
        if 'carrier_offset_hz' in features:
            note_parts.append(f"Î”f={features['carrier_offset_hz']:.0f}Hz")
    ...
    def track_transmission(self, freq_hz, modulation, symbol_rate,
        ...
        if op_sig not in self.db:
            ...
            if modulation == '?' and not bearing_cluster:
                return None  # skip pure noise â€” don't create phantom operators
            self.db[op_sig] = {
                'first_seen': now,
...
class PLLResonanceTransmissionDetector:
    """
    True Costas-loop PLL for RF carrier â†’ audio correlation.
...
class eCPRIInjectionDetector:
    ...
    def detect(self):
        ...
        # FFT-based autocorrelation (O(N log N) vs O(NÂ²) for np.correlate)
        N = len(power)
        if N > 100000:
            fft_power = fft(power)
...
class MapHandler(BaseHTTPRequestHandler):
    ...
    def _do_GET(self):
        ...
        elif _p == '/camera/rf' or _p == '/camera/rf.jpg':
            # Quad RF camera overlay: render RF detection data on live camera feed.
            # Shows bearing markers, signal sources, and HUD overlaid on the camera image.
            if not self._is_local():
                self.send_error(403); return
            try:
                from camera_rf_overlay import get_camera_rf_image
                with MapHandler._data_lock:
                    data = dict(MapHandler.detections_data)
                img_data = get_camera_rf_image(data)
                if img_data:
                    self.send_response(200)
                    self.send_header('Content-Type', 'image/jpeg')
                    self.send_header('Cache-Control', 'max-age=2')
                    self.send_header('X-Camera-Status', 'rf-overlay')
                    self.send_header('Content-Length', str(len(img_data)))
                    self.end_headers()
                    self.wfile.write(img_data)
                else:
                    self.send_error(503, 'RF overlay unavailable - no camera image')
            except Exception as _rf_e:
                self.send_error(500, f'RF overlay render error: {_rf_e}')
            return
        ...
        if self.path == '/' or self.path == '/map':
        ...
        elif self.path == '/api/jammer' or self.path.startswith('/api/jammer?'):
            import urllib.parse
            qs = urllib.parse.urlparse(self.path).query
            params = urllib.parse.parse_qs(qs)
            action = params.get('action', [None])[0]

            # Handle parametric jammer toggle
            tscm = get_tscm_instance()
            if tscm and hasattr(tscm, 'us_jammer'):
                pj = tscm.us_jammer
                if action == 'enable':
                    pj.auto_mode = False  # Manual control
                    pj.enable()
                elif action == 'disable':
                    pj.auto_mode = False
                    pj.disable()
            ...
            data = {
                ...
                'clips': clips,
                'parametric': {
                    'enabled': pj.enabled if (tscm and hasattr(tscm, 'us_jammer')) else False,
                    'auto_mode': getattr(pj, 'auto_mode', False) if (tscm and hasattr(tscm, 'us_jammer')) else False,
                    'effectiveness': pj.effectiveness if (tscm and hasattr(tscm, 'us_jammer')) else 0.0,
                    'level': pj.level if (tscm and hasattr(tscm, 'us_jammer')) else 0.0,
                }
            }
...
class GPSInterface:
    ...
    def _configure_anti_spoofing(self, ser):
        ...
        try:
            ...
            # Raw UBX fallback: CFG-PRT for UART1 â†’ NMEA output
            try:
...
class HackRFSubprocess:
    ...
    def start(self, duration_ms=200):
        ...
        if result.returncode!=0:
            print("âš ï¸ hackrf_transfer not found, trying PyUSB"); self._start_pyusb(duration_ms); return
        self.active=True
        def capture():
            while self.active:
                try:
    ...
    def _start_pyusb(self, duration_ms):
        try:
            ...
            print("âœ… HackRF PyUSB bridge active")
        except ImportError:
            print("âŒ Neither hackrf_transfer nor hackrf_usb.py available"); self.active=False
...
class RTLSDRCapture:
    ...
    Uses ctypes to call librtlsdr_w64_static.dll directly â€” no rtl_tcp, no libusb
    ...
    def _open_ctypes(self):
        ...
        if result != 0:
            sys.stderr.write(f"âŒ RTL-SDR: rtlsdr_open failed (error {result}) â€” driver may be locked by DVB-T driver. "
    ...
    def start(self, duration_ms=200):
        ...
        if self._open_ctypes():
            ...
            sys.stderr.write(f"âœ… RTL-SDR active (ctypes/static DLL): {self.frequency/1e6:.0f} MHz {self.sample_rate/1e6:.1f} MSps gain={self.gain}\n"); sys.stderr.flush()
            def capture_ctypes():
                ...
                while self.active:
                    try:
                        ...
                        if consecutive_failures > 5:
                            sys.stderr.write(f"âŒ RTL-SDR ctypes read failed 5x: {e}\n"); sys.stderr.flush()
                            self.active = False
                            break
                        time.sleep(1)
                        try:
                        ...
                        if not self._open_ctypes():
                            sys.stderr.write("âŒ RTL-SDR ctypes reconnect failed\n"); sys.stderr.flush()
                            self.active = False
                            break
                    time.sleep(0.01)
        ...
        try:
            ...
            self.active = True
            samples_to_read = max(1024, int(self.sample_rate * duration_ms / 1000))
            sys.stderr.write(f"âœ… RTL-SDR active (rtlsdr lib): {self.frequency/1e6:.0f} MHz {self.sample_rate/1e6:.1f} MSps gain={self.gain}\n"); sys.stderr.flush()
            ...
            def capture():
                ...
                while self.active:
                    try:
                        ...
                        consecutive_failures += 1
                        if consecutive_failures > 5:
                            sys.stderr.write(f"âŒ RTL-SDR read failed 5 times: {e}\n"); sys.stderr.flush()
                            self.active = False
                            break
                        time.sleep(1)
                        try:
                        ...
                        try:
                            ...
                            print("âœ… RTL-SDR reconnected", flush=True)
                            consecutive_failures = 0
                        except Exception as re:
                            print(f"âŒ RTL-SDR reconnect failed: {re}")
...
class BladeRFCLIBridge:
    ...
    """
    # Expected capture: n=1048576 MIMO samples (2ch Ã— I,Q Ã— 2 bytes int16 = 4194304 bytes)
    ...
    def _validate_iq(self, iq1, iq2):
        ...
        if self.last_iq1_rms > 0:
            ...
            if rms_change > 2.0:  # More than 2x change
                warnings.append(f"rms_spike_ch1: {rms_change:.1f}x change ({self.last_iq1_rms:.0f}â†’{rms1:.0f})")
        if self.last_iq2_rms > 0:
            rms_change = abs(rms2 - self.last_iq2_rms) / self.last_iq2_rms
            if rms_change > 2.0:
                warnings.append(f"rms_spike_ch2: {rms_change:.1f}x change ({self.last_iq2_rms:.0f}â†’{rms2:.0f})")
    ...
    def start(self):
        result=subprocess.run(['where','bladeRF-cli'],capture_output=True,text=True,timeout=5)
        if result.returncode!=0: print("bladeRF-cli not found"); return False
        ...
        if fw_info:
            ...
            print(f"ðŸ”’ BladeRF firmware hash: {self.firmware_hash}")
        ...
        def capture_loop():
            while self.active:
                try:
                    # Bias tee is enabled in the per-capture script (set biastee rx1/rx2 on)
                    # which runs inside the same bladeRF-cli -s session. No separate process needed.
                    ...
                    # Bias tees: enable RX1+RX2 for LNA power (2 active, safe limit is 2)
                    # TX bias tee stays off — enabling all 3 in script mode crashes firmware
                    cmd_script = (f"set biastee rx1 on\n"
                                f"set biastee rx2 on\n"
                    ...
                    if os.path.exists(tmpname):
                        ...
                        if not valid:
                            ...
                            print(f"ðŸš¨ CAPTURE INTEGRITY FAILURE: {reason}")
                            os.unlink(tmpname)
                            time.sleep(0.3)
                            continue
        ...
        print("âœ… BladeRF CLI bridge active (hardened)"); return True
...
class LaptopMic:
    ...
    def start(self):
        ...
        if self.device_index is None: print("âš ï¸ Laptop mic not found"); return
        ...
        for ch in [4, 2, 1]:
            try:
                ...
                print(f"âœ… Laptop mic on device {self.device_index} ({ch}ch)"); return
            except: continue
        print(f"âŒ Laptop mic failed on device {self.device_index}")
    def _cb(self, indata, frames, time_info, status):
        if self._channels >= 4:
...
class WiFiUDPListener:
    def __init__(self, port=9999, callback=None):
        ...
        # Bind to localhost ONLY â€” prevents external devices from injecting fake detections
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    continue
                # Basic schema validation â€” must have detector field and is a dict
...
class USBWatchdog:
    def __init__(self, vid, pid, callback=None):
        ...
        if WMI_AVAILABLE:
            ...
            threading.Thread(target=self._loop,daemon=True).start()
        else: print("âš ï¸ WMI not available, USB watchdog disabled")
...
class GLMWatchdog:
    ...
    def validate(self, new_target):
        ...
        if len(set(recent))>self.max_changes:
            self.log.warning(f"âš ï¸ GLM erratic"); return False
...
class AdaptiveCoherenceController:
    ...
    def set_rf_target(self, freq):
        self.rf_target=freq
        if freq: self.log.info(f"ðŸŽ¯ RF cancel target: {freq/1e6:.3f} MHz")
    def set_vlf_audio_target(self, freq):
        self.audio_target=freq
        if freq: self.log.info(f"ðŸ”Š VLF cancel target: {freq:.1f} Hz")
    def _run(self):
        while self.running:
            try:
    ...
    def _enable_wifi_dongle(self):
        ...
        try:
            ...
            if '8812' in result.stdout:
                # Device exists â€” try to enable it
                ...
                self.log.info("WiFi dongle (8812AU) detected â€” attempting restart")
            self._enable_alfa()
        except Exception:
            pass
    ...
    def _scan(self):
        ...
        try:
            ...
            if self.access_points:
                self.log.info(f"ðŸ“¶ WiFi scan: {len(self.access_points)} APs found")
            ...
            try:
                if self.gps and self.gps.has_fix:
                    if self.gps.last_update>0 and abs(time.time()-self.gps.last_update)>1.0:
                        self.log.warning("â±ï¸ Clock skewed")
...
def classify_detection(detector_name, freq=0.0):
    ...
    PHYSICS:
    - Microwave voice attack â†’ hits victim's body â†’ carbon square-law interaction
      â†’ produces ultrasound/audio FROM the victim's body at the modulation frequency.
    ...
    - pll_resonance_transmission / forced_thought = RF-AUDIO CROSS-CORRELATION MATCH
      â†’ This is PROOF that a specific RF carrier is causing the audio.
      â†’ The freq field is the RF CARRIER frequency of the TRANSMITTER.
    - injection_locking, silent_sound, power_line_loop, eardrum_capture = audio/ultrasound
      detected at the victim's position â†’ these are VICTIM markers.
    ...
    # RF carrier â†’ audio cross-correlation = TRANSMITTER (the MW source)
...
class BladeRFTXBridge:
    ...
    def start_noise_jam(self, center_freq, bandwidth_hz=20e6, gain=60):
        """Wideband noise jamming: transmit Gaussian noise for saturation disruption.
        More effective than CW null steering against frequency-hopping/spread-spectrum signals."""
        if self.active: self.stop_tx()
        try:
            self.tx_freq = center_freq; self.tx_gain = gain
            nsamples = 65536
            rng = np.random.RandomState(int(time.time() * 1e6) % 2**31)
            iq_i = (rng.randn(nsamples) * 1500).astype(np.int16)
            iq_q = (rng.randn(nsamples) * 1500).astype(np.int16)
            interleaved = np.empty(nsamples*2, dtype=np.int16)
            interleaved[0::2] = iq_i; interleaved[1::2] = iq_q
            txfile = os.path.join(tempfile.gettempdir(), 'tscm_tx_noise.sc16q11')
            interleaved.tofile(txfile)
            sample_rate = int(bandwidth_hz)
            cmd_script = (
                f"set biastee rx1 off\n"
                f"set biastee rx2 off\n"
                f"set frequency tx {int(center_freq)}\n"
                f"set samplerate tx {sample_rate}\n"
                f"set bandwidth tx {int(bandwidth_hz * 0.8)}\n"
                f"set gain tx {gain}\n"
                f"tx config file={txfile} format=bin channels=1 repeat=0\n"
                f"tx start\n"
            )
            self.process = subprocess.Popen(
                ['bladeRF-cli', '-i'],
            ...
            print(f"BladeRF noise jam error: {e}")
            self.active = False
            return False
...
class ActiveNullSteering:
    ...
    def __init__(self, bladerf_rx, bladerf_tx=None):
        ...
        self.null_freqs = {}  # freq â†’ {phase, power, last_adjust}
        self.enabled = False
        self.max_null_depth_db = 0
        self.null_history = deque(maxlen=100)
        # TDM jamming: cycle through multiple threat frequencies
        self._tdm_targets = []    # [(freq, priority), ...]
        self._tdm_index = 0
        self._tdm_last_switch = 0
        self._tdm_dwell_ms = 500  # dwell 500ms per frequency before switching

    def set_tdm_targets(self, targets):
        self._tdm_targets = sorted(targets, key=lambda x: x[1], reverse=True)

    def _tdm_cycle(self):
        if not self._tdm_targets or not self.enabled:
            return
        now = time.time()
        if now - self._tdm_last_switch < self._tdm_dwell_ms / 1000.0:
            return
        self._tdm_last_switch = now
        self._tdm_index = (self._tdm_index + 1) % len(self._tdm_targets)
        freq, _ = self._tdm_targets[self._tdm_index]
        if freq > 0:
            self.tx.start_tx(freq, gain=60)
    ...
    def update(self, attacker_freq, attacker_bearing, rx_power):
        ...
        # Phase sweep Â±10deg  to find minimum RX power
        ...
        # Time-division multiplex: cycle TX to next jam target
        self._tdm_cycle()
...
class LoopAntennaTX:
    """
    Body-field cancellation via headphone audio â†’ amplifier â†’ loop antenna.
    ...
    3. Plays through headphones â†’ amp â†’ loop antenna
...
if TORCH_AVAILABLE:
    ...
    class NeuralDetector:
        ...
        def __init__(self, path='models/signalnet.pt'):
            ...
            self.count = 0
            if os.path.exists(path):
                try: self.model.load_state_dict(torch.load(path, map_location=self.device)); print(f"ðŸ§  SignalNet loaded ({self.device})")
                except: print(f"ðŸ§  SignalNet fresh ({self.device})")
            else: print(f"ðŸ§  SignalNet fresh ({self.device})")
...
class ThreatScoringEngine:
    ...
    def score_source(self, fingerprint, classification, snr, freq, bearing,
        ...
        # 2. SNR strength (0-20 points) â€” stronger = closer = more threat
        if snr and snr > 0:
            score += min(20, (snr / 30.0) * 20)

        # 3. Observation persistence (0-15 points) â€” more observations = more persistent
        if observations:
...
class CycleAnomalyDetector:
    ...
    noise floor â€” indicators of new attack vectors or equipment changes."""
...
class RFBurstDetector:
    ...
    within 2 seconds â€” indicates a device powering on or starting transmission.
...
class HiddenSignalHunter:
    ...
    Detection: compare same-detector pairs â€” if one is >3x stronger AND
...
# ===================== DECOY SUBTRACTION ENGINE =====================
# Digitally removes high-power decoy signals from raw IQ to reveal
# hidden low-power sources buried under the decoy sidelobes.
#
# This is fundamentally different from HiddenSignalHunter (which compares
# already-detected sources). DecoySubtractionEngine works BEFORE detection
# at the RAW IQ LEVEL:
#   1. FFT-based decoy identification (high-SNR peaks)
#   2. Adaptive IIR notch filter bank (one notch per stable decoy)
#   3. Apply all notches to IQ → residue (what's left after decoy removal)
#   4. Re-scan residue at LOWER thresholds to find hidden signals
#   5. Cross-sensor coherence: weak signal on BladeRF+HackRF+RTL = real
#
# Stable decoy criteria: must be seen at consistent frequency for 3+ cycles.
# This prevents accidentally notching a transient strong signal that isn't a decoy.

class DecoySubtractionEngine:
    """Digitally remove high-power decoy signals to reveal hidden low-power sources."""

    def __init__(self, log_func=None):
        self.log = log_func or (lambda msg: None)
        self.strong_threshold_db = 12.0  # SNR ratio to flag as "strong" decoy candidate
        self.residue_threshold_mult = 1.6  # peak threshold multiplier for residue scan (lower = more sensitive)
        self.notch_q = 35.0  # Q factor for notch filters (higher = narrower notch, less collateral damage)
        self.max_notches = 10  # Max simultaneous notch filters
        self.stability_cycles = 3  # Must be seen this many cycles to be considered stable

        # Active decoy profiles: freq_key -> {freq, snr, bw, first_seen, last_seen, count, stable}
        self.decoy_profiles = {}

        # Residue findings tracking
        self.residue_history = deque(maxlen=200)

        # Cross-sensor coherence: track which hidden frequencies appear on multiple sensors
        self.sensor_hits = defaultdict(list)  # freq_bin -> [(sensor, snr_db, ts), ...]
        self.coherence_window_s = 8.0  # Must appear on 2+ sensors within this window

        # Adaptive residue threshold
        self.residue_threshold_min = 1.2   # Absolute minimum (most aggressive)
        self.residue_threshold_max = 2.2   # Absolute maximum (most conservative)
        self.residue_threshold_adapt_rate = 0.08  # Adjustment per cycle
        self.consecutive_empty_cycles = 0   # Counter for no-finding cycles
        self.consecutive_flood_cycles = 0    # Counter for too-many-finding cycles
        self.empty_threshold = 5   # After this many empty cycles, start lowering
        self.flood_threshold = 10  # Findings per cycle above this = flooding

        # Statistics
        self.total_decoys_removed = 0
        self.total_hidden_found = 0
        self.cycle_coherent_finds = []  # Last cycle's cross-sensor hits

    def design_notch_filter(self, freq_hz, fs, q=35.0):
        """Design a narrow IIR notch filter at a specific frequency."""
        w0 = freq_hz / (fs / 2)
        if w0 >= 0.99 or w0 <= 0.001:
            return None  # Frequency too close to DC or Nyquist
        try:
    ...
    def characterize_decoys(self, iq_data, center_freq, sample_rate, sensor_name='unknown'):
        """Identify strong signals in IQ that may be decoys masking weaker signals.

        Returns: list of (freq_hz, snr_db, bandwidth_hz) for each strong signal
        """
        if iq_data is None or len(iq_data) < 256:
            return []

        n = len(iq_data)
        fft = np.abs(np.fft.rfft(iq_data))
        freqs = np.fft.rfftfreq(n, 1 / sample_rate) + center_freq
        noise_floor = np.median(fft[10:]) + 1e-12  # Skip DC bin

        # Find peaks above the strong threshold
        threshold = noise_floor * self.strong_threshold_db
        peaks, props = find_peaks(fft, height=threshold, distance=8)

        strong_signals = []
        now = time.time()
        for pk in peaks[:self.max_notches]:
            snr_linear = fft[pk] / noise_floor
            snr_db = 20.0 * np.log10(max(snr_linear, 1.0))
            freq = freqs[pk]

            # Estimate bandwidth (-3dB width)
            peak_height = fft[pk]
            half_height = peak_height / np.sqrt(2)

            left_idx = int(pk)
            while left_idx > 1 and fft[left_idx - 1] > half_height:
                left_idx -= 1
            right_idx = int(pk)
            while right_idx < len(fft) - 1 and fft[right_idx + 1] > half_height:
                right_idx += 1

            bw_hz = freqs[min(right_idx, len(freqs) - 1)] - freqs[max(left_idx, 0)]
            bw_hz = max(bw_hz, sample_rate / n * 2)  # At least one FFT bin width

            strong_signals.append((freq, snr_db, bw_hz))

            # Update decoy profile (1 kHz bins for sub-GHz, 10 kHz for >1GHz)
            if freq > 1e9:
                key = round(freq / 10000) * 10000
            else:
                key = round(freq / 1000) * 1000

            if key not in self.decoy_profiles:
                self.decoy_profiles[key] = {
                    'freq': freq, 'snr': snr_db, 'bw': bw_hz,
                    'first_seen': now, 'last_seen': now,
                    'count': 0, 'stable': False, 'sensors': {sensor_name},
                    'notch_coeffs': None
                }
            p = self.decoy_profiles[key]
            # Frequency stability check: only count if freq is close to previous
            freq_diff = abs(freq - p['freq'])
            if freq_diff < max(bw_hz * 2, sample_rate / n * 5):
                p['snr'] = p['snr'] * 0.7 + snr_db * 0.3
                p['bw'] = p['bw'] * 0.7 + bw_hz * 0.3
                p['freq'] = p['freq'] * 0.9 + freq * 0.1  # Smooth frequency estimate
                p['last_seen'] = now
                p['count'] += 1
                p['sensors'].add(sensor_name)
                if p['count'] >= self.stability_cycles:
                    p['stable'] = True
                    # Pre-compute notch coefficients for this stable decoy
                    bb_freq = p['freq'] - center_freq
                    if abs(bb_freq) < sample_rate / 2 * 0.95:
                        coeffs = self.design_notch_filter(bb_freq, sample_rate, q=self.notch_q)
                        if coeffs is not None:
                            p['notch_coeffs'] = (coeffs, center_freq, sample_rate)

        # Purge stale profiles (>120s not seen)
        stale = [k for k, v in self.decoy_profiles.items() if now - v['last_seen'] > 120]
        for k in stale:
            del self.decoy_profiles[k]

        return strong_signals

    def subtract_decoys(self, iq_data, center_freq, sample_rate, sensor_name='unknown'):
        """Apply notch filters for all stable decoys, return cleaned IQ.

        Only subtracts STABLE decoys (seen consistently for stability_cycles+).

        Returns: (cleaned_iq, removed_count, residue_findings)
        """
        if iq_data is None or len(iq_data) < 256:
            return iq_data, 0, []

        # Characterize current signal environment (updates decoy profiles)
        self.characterize_decoys(iq_data, center_freq, sample_rate, sensor_name)

        # Get stable decoys (seen consistently)
        stable_decoys = [(k, v) for k, v in self.decoy_profiles.items() if v['stable']]
        if len(stable_decoys) == 0:
            return iq_data, 0, []

        # Apply notch filters sequentially (filter the original, not cumulative)
        cleaned = iq_data.astype(np.float64).copy()
        removed = 0

        for key, profile in stable_decoys:
            coeffs_info = profile.get('notch_coeffs')
            if coeffs_info is None:
                continue
            (b, a), saved_cf, saved_fs = coeffs_info
            # Only use if center frequency and sample rate match current capture
            # (prevents applying HackRF notches to BladeRF data with wrong params)
            if abs(saved_cf - center_freq) < sample_rate * 0.1 and abs(saved_fs - sample_rate) < sample_rate * 0.1:
                try:
                    # filtfilt = zero-phase (forward-backward), no phase distortion
                    cleaned = scipy.signal.filtfilt(b, a, cleaned)
                    removed += 1
                except Exception:
                    pass

        # Scan cleaned IQ for residual (hidden) signals
        residue_findings = []
        if removed > 0:
            n = len(cleaned)
            res_fft = np.abs(np.fft.rfft(cleaned))
            res_freqs = np.fft.rfftfreq(n, 1 / sample_rate) + center_freq
            res_noise = np.median(res_fft[10:]) + 1e-12

            # LOWER threshold for residue (more sensitive)
            res_threshold = res_noise * self.residue_threshold_mult
            res_peaks, _ = find_peaks(res_fft, height=res_threshold, distance=10)

            # Build set of decoy frequencies to exclude
            decoy_freqs = set()
            for k, v in stable_decoys:
                decoy_freqs.add(round(v['freq']))

            now = time.time()
            for pk in res_peaks[:15]:
                freq = res_freqs[pk]
                rounded = round(freq)
                # Skip if this matches a decoy frequency we just removed
                is_hidden = True
                for df in decoy_freqs:
                    if abs(rounded - df) < 2000:  # Within 2kHz of a removed decoy
                        is_hidden = False
                        break

                if is_hidden:
                    snr_linear = res_fft[pk] / res_noise
                    snr_db = 20.0 * np.log10(max(snr_linear, 1.0))
                    residue_findings.append({
                        'freq': freq, 'snr_db': float(snr_db),
                        'sensor': sensor_name, 'ts': now,
                        'n_decoys_removed': removed
                    })

                    # Cross-sensor coherence tracking
                    freq_key = round(freq / 1000) * 1000
                    self.sensor_hits[freq_key].append((sensor_name, float(snr_db), now))

            # Store findings
            for rf in residue_findings:
                self.residue_history.append(rf)
            self.total_hidden_found += len(residue_findings)

        # Adaptive threshold adjustment after this scan
        self.adapt_threshold(residue_findings)

        self.total_decoys_removed += removed
        return cleaned, removed, residue_findings

    def check_cross_sensor_coherence(self):
        """Find frequencies that appear on 2+ different sensors = real hidden signals."""
        now = time.time()
        coherent = []
        stale_keys = []

        for freq_key, hits in self.sensor_hits.items():
            # Purge old hits
            active_hits = [h for h in hits if now - h[2] < self.coherence_window_s]
            if len(active_hits) < 2:
                if len(active_hits) == 0:
                    stale_keys.append(freq_key)
                continue

            # Check unique sensors
            sensors = set(h[0] for h in active_hits)
            if len(sensors) >= 2:
                avg_snr = float(np.mean([h[1] for h in active_hits]))
                coherent.append({
                    'freq': freq_key,
                    'snr_db': avg_snr,
                    'sensors': sorted(sensors),
                    'n_hits': len(active_hits),
                    'ts': now
                })

        for k in stale_keys:
            del self.sensor_hits[k]

        self.cycle_coherent_finds = coherent
        return coherent

    def adapt_threshold(self, findings_this_cycle):
        """Auto-adjust residue threshold based on detection performance.

        Too many findings = possible false positives = raise threshold.
        Too few findings = possible missed signals = lower threshold.
        Cross-sensor coherent hits validate current threshold.
        """
        n_findings = len(findings_this_cycle)
        n_coherent = len(self.cycle_coherent_finds)

        # If we have cross-sensor coherent hits, threshold is good - hold steady
        if n_coherent > 0:
            self.consecutive_empty_cycles = 0
            self.consecutive_flood_cycles = 0
            return

        if n_findings == 0:
            self.consecutive_empty_cycles += 1
            self.consecutive_flood_cycles = 0
            # After N empty cycles, start lowering threshold (be more sensitive)
            if self.consecutive_empty_cycles >= self.empty_threshold:
                old_t = self.residue_threshold_mult
                self.residue_threshold_mult = max(
                    self.residue_threshold_min,
                    self.residue_threshold_mult - self.residue_threshold_adapt_rate
                )
                if abs(self.residue_threshold_mult - old_t) > 0.01:
                    self.log('DECOY-ADAPT: no hidden signals for %d cycles, lowering residue threshold %.2f -> %.2f' % (
                        self.consecutive_empty_cycles, old_t, self.residue_threshold_mult))
        elif n_findings >= self.flood_threshold:
            self.consecutive_flood_cycles += 1
            self.consecutive_empty_cycles = 0
            if self.consecutive_flood_cycles >= 3:
                old_t = self.residue_threshold_mult
                self.residue_threshold_mult = min(
                    self.residue_threshold_max,
                    self.residue_threshold_mult + self.residue_threshold_adapt_rate
                )
                if abs(self.residue_threshold_mult - old_t) > 0.01:
                    self.log('DECOY-ADAPT: flooding (%d findings), raising residue threshold %.2f -> %.2f' % (
                        n_findings, old_t, self.residue_threshold_mult))
        else:
            # Normal operation - some findings but not flooded
            self.consecutive_empty_cycles = max(0, self.consecutive_empty_cycles - 1)
            self.consecutive_flood_cycles = max(0, self.consecutive_flood_cycles - 1)

    def get_stats(self):
        """Return summary stats for map dashboard and logging."""
        stable_count = len([v for v in self.decoy_profiles.values() if v['stable']])
        all_decoy_count = len(self.decoy_profiles)
        return {
            'active_decoys': stable_count,
            'candidate_decoys': all_decoy_count - stable_count,
            'total_decoys_removed': self.total_decoys_removed,
            'total_hidden_found': self.total_hidden_found,
            'recent_residue': list(self.residue_history)[-30:],
            'cross_sensor_hits': self.cycle_coherent_finds,
            'decoy_profiles': [
                {'freq': v['freq'], 'snr': round(v['snr'], 1),
                 'bw': round(v['bw']), 'count': v['count'],
                 'stable': v['stable'], 'sensors': sorted(v['sensors'])}
                for v in sorted(self.decoy_profiles.values(),
                                key=lambda x: x['snr'], reverse=True)[:10]
            ]
        }
...
# Global TSCM instance for API access (jammer toggle, etc.)
_tscm_instance = None

def get_tscm_instance():
    return _tscm_instance

class TSCMSystem:
    def __init__(self):
        global _tscm_instance
        _tscm_instance = self
        ...
        self.detectors={
            ...
            # Parametric countermeasures (anti-parametric-amplification)
            'beat_freq':UltrasonicBeatFrequencyDetector(),
        }
        # Cross-layer parametric attack correlator
        self.cross_layer = CrossLayerParametricCorrelator(window_s=10.0, min_layers=2)
        # Ultrasonic noise jammer (DISABLED by default — requires explicit enable)
        self.us_jammer = UltrasonicNoiseJammer(output_fs=48000, band=(20000, 23500))
        ...
        # Wiggle Orchestrator â€” six-axis randomization for anti-pattern defense
        ...
        # Decoy subtraction engine: remove high-power decoys to reveal hidden signals
        self.decoy_subtraction = DecoySubtractionEngine(log_func=self.log.info)
        ...
        if self.rtlsdr:
            try:
                ...
                def _retry_rtlsdr():
                    ...
                    self.log.info("RTL-SDR: not detected after 3 retries â€” third sensor offline")
        ...
        if self.bladerf_cli:
            # Start BladeRF in background thread with timeout — don't block TSCM startup
            import threading as _bt
            def _bg_bladerf():
                try:
                    started = self.bladerf_cli.start()
                    if started:
                        print("BladeRF online in background")
                    else:
                        print("BladeRF init failed — falling back to HackRF+RTL-SDR")
                        self.bladerf_cli = None
                except Exception as e:
                    print(f"BladeRF init error: {e} — falling back")
                    self.bladerf_cli = None
            _bt.Thread(target=_bg_bladerf, daemon=True).start()
            # Give BladeRF 8 seconds to initialize, then proceed regardless
            time.sleep(0.5)  # Brief yield for thread to start
    ...
    # GPS cache to avoid re-opening serial on every cycle
    _gps_serial = None
    _gps_last_port = None
    _gps_last_lat = None
    _gps_last_lon = None
    ...
    def _read_gps_position(self, port, timeout=0.5):
        """Read GGA sentence from GPS on serial port. Returns (lat, lon) or (None, None).
        Caches serial connection across cycles for efficiency.
        Anti-spoof: validates lat/lon bounds, fix quality >= 1."""
        now = time.time()
        # Return cached position if read within last 2 seconds
        if now - self._gps_last_ts < 2 and self._gps_last_lat:
            return self._gps_last_lat, self._gps_last_lon
        try:
            import serial
            if self._gps_serial is None or self._gps_last_port != port or not self._gps_serial.is_open:
                if self._gps_serial:
                    try: self._gps_serial.close()
                    except: pass
                self._gps_serial = serial.Serial(port, Config.GPS_BAUD, timeout=timeout)
                self._gps_last_port = port
            ser = self._gps_serial
            ser.reset_input_buffer()
            t0 = time.time()
            while time.time() - t0 < 1.5:
                line = ser.readline()
                try: line = line.decode('ascii', errors='ignore').strip()
                except: continue
                ...
                fix_quality = parts[6]
                if fix_quality not in ('1', '2', '4', '5'): continue  # no fix
                lat_str = parts[2]; lat_dir = parts[3]
                lon_str = parts[4]; lon_dir = parts[5]
                ...
                try:
                    ...
                    continue
                if abs(lat_dd) < 0.1 or abs(lon_dd) < 0.1: continue
                self._gps_last_lat = lat_dd
                self._gps_last_lon = lon_dd
                self._gps_last_ts = now
                return lat_dd, lon_dd
        except Exception as e:
            # GPS port unavailable — clean up cached serial
            if self._gps_serial:
                try: self._gps_serial.close()
                except: pass
            self._gps_serial = None
            self._gps_last_port = None
        return None, None
    ...
    def _compute_aoa(self, iq1, iq2, freq):
        ...
        try:
            ...
            aoa = (aoa + Config.BEARING_OFFSET + 180 + 360) % 360  # 180deg flip for correct hemisphere
            ...
            # (plane wave from ch1 side â†’ ch2 first). Negative = ch2 side.
            ...
            # Phase sign + 1 â†’ source is on the positive-rotation side of array axis
            # Phase sign - 1 â†’ source is on the negative-rotation side
            ...
            # â†’ source is on RX2 side â†’ clockwise from array axis.
            ...
            if power_ratio > 1.15:
                ...
                if abs(((aoa - side1 + 180) % 360) - 180) < abs(((aoa_alt - side1 + 180) % 360) - 180):
                    ...
                    resolved = aoa_alt
                self.log.info(f"AoA RESOLVED(power): {aoa}â†’{resolved}deg (ch1 stronger, p1/p2={power_ratio})")
                aoa = resolved
                ...
                if abs(((aoa - side2 + 180) % 360) - 180) < abs(((aoa_alt - side2 + 180) % 360) - 180):
                    ...
                    resolved = aoa_alt
                self.log.info(f"AoA RESOLVED(power): {aoa}â†’{resolved}deg (ch2 stronger, p1/p2={power_ratio})")
                aoa = resolved
            else:
                # Power ambiguous (p1/p2 ~ 1.0) â€” try phase sign and history
                ...
                if hist_aoa is not None:
                    ...
                    if d2 < d1:
                        resolved = aoa_alt
                        self.log.info(f"AoA RESOLVED(history): {aoa}â†’{resolved}deg (hist={hist_aoa}deg)")
                        aoa = resolved
                    else:
                        self.log.info(f"AoA RESOLVED(history): kept {aoa}deg (hist={hist_aoa}deg)")
                elif coherence > 0.2:
                    # Use phase sign: positive phase â†’ RX2 leads â†’ source on RX1 side
                    if phase_diff > 0:
                    ...
                    if abs(((aoa - side + 180) % 360) - 180) < abs(((aoa_alt - side + 180) % 360) - 180):
                        ...
                        resolved = aoa_alt
                    self.log.info(f"AoA RESOLVED(phase): {aoa}â†’{resolved}deg (phase={np.degrees(phase_diff)}deg coh={coherence})")
                    aoa = resolved
                else:
    ...
    def _on_wifi_detection(self, det):
        ...
        if processed: self.log.warning(f"ðŸ“¶ High-Power Wi-Fi: {processed}")

    def _on_usb_event(self, msg):
        self.log.warning(f"ðŸ”Œ USB: {msg}")
    ...
    def _process_detection_with_localization(self, det, obs_lat, obs_lon, aoa, source_type='rf', sdr_source='bladerf'):
        ...
        # BladeRF AoA only applies to BladeRF band (2.4 GHz Â± 5 MHz).
        ...
        if aoa != 0.0 and (_in_bladerf_band or freq > 100):
            ...
            # If no bistatic range, leave range_m=None â€” resolve_sources will
    ...
    def run(self):
        self.running=True
        self.log.info("ðŸ›¡ï¸ TSCM Source Localization Suite active")
        ...
        while self.running:
            ...
            # === RTL-SDR AUTO-HEAL: reconnect dongle if it dropped (replug recovery) ===
            if self.rtlsdr is not None and not getattr(self.rtlsdr, 'active', False) and self.cycle_count % 15 == 0:
                try:
                    if self.rtlsdr.start(duration_ms=200):
                        self.log.info("RTL-SDR auto-healed (reconnected after replug)")
                except Exception:
                    pass
            ...
            # 1. Position — GPS primary (ZED-F9P), fixed fallback
            # Uses existing GPSInterface (already tracking fix status)
            # Anti-spoof: cross-check against HOME, reject jumps >500m
            lat, lon = Config.HOME_LAT, Config.HOME_LON
            gps_fix = False
            gps_source = 'fixed'
            if hasattr(self, 'gps') and self.gps and self.gps.has_fix:
                glat = self.gps.lat
                glon = self.gps.lon
                if glat and glon and abs(glat) > 0.01:
                    dy = (glat - Config.HOME_LAT) * 111320.0
                    dx = (glon - Config.HOME_LON) * 111320.0 * math.cos(math.radians(Config.HOME_LAT))
                    jump_m = math.sqrt(dx*dx + dy*dy)
                    if jump_m < 500:
                        lat, lon = glat, glon
                        gps_fix = True
                        gps_source = 'gps_zed-f9p'
                    else:
                        self.log.warning(f'GPS SPOOF: {jump_m:.0f}m jump rejected')
                        gps_source = 'gps_spoof_suspect'
                else:
                    gps_source = 'gps_no_valid_position'
            gps = {'has_fix': gps_fix, 'lat': lat, 'lon': lon, 'source': gps_source}
            ...
            if bladerf_active:
                ...
                # DECOY SUBTRACTION: remove strong carriers from BOTH channels, scan residue for hidden signals
                if bladerf_active and iq1 is not None and len(iq1) > 1024:
                    try:
                        # Clean channel 1
                        cleaned_iq, n_removed, residue = self.decoy_subtraction.subtract_decoys(
                            iq1, Config.BLADERF_FREQ, Config.BLADERF_SAMPLE_RATE, sensor_name='bladerf')
                        # Clean channel 2 (same decoy profile, different IQ) for residue AoA
                        residue_aoa = None
                        if n_removed > 0 and iq2 is not None and len(iq2) > 1024:
                            try:
                                cleaned_iq2, _, _ = self.decoy_subtraction.subtract_decoys(
                                    iq2, Config.BLADERF_FREQ, Config.BLADERF_SAMPLE_RATE, sensor_name='bladerf_ch2')
                                # Compute AoA from cleaned residue on BOTH channels
                                if len(cleaned_iq) > 1024 and len(cleaned_iq2) > 1024:
                                    res_aoa_raw, res_coherence = self._compute_aoa(
                                        cleaned_iq[-4096:], cleaned_iq2[-4096:], Config.BLADERF_FREQ)
                                    if res_coherence is not None and res_coherence > 0.04:
                                        residue_aoa = float(res_aoa_raw)
                            except Exception:
                                pass
                        if residue:
                            decoy_stats = self.decoy_subtraction.get_stats()
                            self.log.info('DECOY-SUB [bladerf]: removed %d decoys, found %d hidden signals (total hidden: %d, active decoys: %d)' % (
                                n_removed, len(residue), decoy_stats["total_hidden_found"], decoy_stats["active_decoys"]))
                            for rf in residue:
                                # Use residue AoA if available, otherwise fall back to original AoA
                                bearing = residue_aoa if residue_aoa is not None else (self.aoa if self.aoa != 0.0 else None)
                                self.localization.add_observation(
                                    fingerprint='decoy_hidden_bladerf_%d' % int(rf['freq']),
                                    obs_lat=lat, obs_lon=lon,
                                    bearing_deg=bearing,
                                    range_m=None, freq=rf['freq'],
                                    classification='hidden_signal',
                                    detector_name='decoy_subtraction_bladerf',
                                    snr=float(rf['snr_db']))
                        if n_removed > 0 and self.cycle_count % 10 == 0:
                            stable_decoys = [(k,v) for k,v in self.decoy_subtraction.decoy_profiles.items() if v['stable']]
                            for _, profile in sorted(stable_decoys, key=lambda x: x[1]['snr'], reverse=True)[:5]:
                                self.log.info('  DECOY [bladerf]: %.3fMHz SNR=%.1fdB BW=%.0fHz seen=%dx sensors=%s' % (
                                    profile['freq']/1e6, profile['snr'], profile['bw'], profile['count'], sorted(profile['sensors'])))
                    except Exception as _dse:
                        if self.cycle_count % 50 == 0:
                            self.log.debug('Decoy subtraction bladeRF error: %s' % str(_dse)[:80])
                ...
                # 2b. CROSS-CORRELATION: audio envelope â†” RF AM envelope (confirm MWâ†’audio)
                if self.cycle_count % 5 == 0 and iq1 is not None and len(mid_chunk) > 2048:
                    try:
                        ...
                        if xc_peak > 0.15:
                            self.log.info(f"MW-AUDIO XCORR: r={xc_peak} (microwaveâ†’audio chain confirmed)")
                ...
                if self.aoa!=0.0:
                    self.log.info(f"ðŸ“¡ AoA: {self.aoa}deg  ({self.aoa_source})"
            ...
            if hack:
                ...
                #   Weak signal = source at null (BEARING_OFFSET Â± 90deg )
                        ...
                        try:
                            ...
                            for pk in peaks[:8]:  # top 8 carriers
                                ...
                                if ratio > 1.5:
                                    ...
                                    # Store for cross-sensor triangulation
                                    self._last_hackrf_aoa = float(peak_bearing)
                                elif ratio < 0.6:
                                    ...
                                    # Store for cross-sensor triangulation
                                    self._last_hackrf_aoa = float(null_bearings[0])
                        except Exception as e:
                ...
                if len(iq_hack) > 1024 and self.cycle_count % 1 == 0:
                    ...
                    hackrf_range = 500.0 / max(hackrf_rms, 0.0001)  # no min clamp, natural inverse-law
                    # Keep MAXIMUM range across sweeps - weakest signal = most distant source.
                    # Prevents local 2.45GHz/0.1MHz self-interference (rms~130, range~30m)
                    # from overwriting the 570MHz ferrite C2 distance (rms~0.016, range~31km).
                    if self.hackrf_range is None or hackrf_range > self.hackrf_range:
                ...
                # _last_hackrf_aoa is captured from ferrite null/peak analysis above
                ...
                if bladerf_active and hackrf_aoa is not None and self.aoa != 0.0:
                    ...
                    if diff > 45:
                        ...
                        self.log.warning(f"âš ï¸ BEARING DIVERGENCE: HackRF={hackrf_bearing:.1f}deg  BladeRF={bladerf_bearing:.1f}deg  diff={diff:.1f}deg ")
                ...
                # DECOY SUBTRACTION on HackRF IQ: remove strong carriers, scan residue
                if iq_hack is not None and len(iq_hack) > 1024:
                    try:
                        cleaned_hack, n_removed_hack, residue_hack = self.decoy_subtraction.subtract_decoys(
                            iq_hack, hack.get('frequency', Config.HACKRF_FREQ_TARGET), Config.HACKRF_SAMPLE_RATE, sensor_name='hackrf')
                        if residue_hack:
                            self.log.info('DECOY-SUB [hackrf]: removed %d decoys, found %d hidden signals at %.1fMHz' % (n_removed_hack, len(residue_hack), hack.get('frequency',0)/1e6))
                            for rf in residue_hack:
                                self.localization.add_observation(
                                    fingerprint='decoy_hidden_hackrf_%d' % int(rf['freq']),
                                    obs_lat=lat, obs_lon=lon,
                                    bearing_deg=hackrf_aoa if hackrf_aoa else None,
                                    range_m=self.hackrf_range, freq=rf['freq'],
                                    classification='hidden_signal',
                                    detector_name='decoy_subtraction_hackrf',
                                    snr=float(rf['snr_db']))
                    except Exception as _dse:
                        if self.cycle_count % 50 == 0:
                            self.log.debug('Decoy subtraction HackRF error: %s' % str(_dse)[:80])
            # 3b. RTL-SDR (Nooelec NESDR Smart) â€” third triangulation sensor
            ...
            # This gives us THREE sensor positions with bearing lines â†’ real triangulation.
            rtlsdr_iq = None
            if self.rtlsdr:
            ...
            if len(ul_chunk)>100:
                ...
                # Feed beat-frequency detector for parametric array detection
                try:
                    self.detectors['beat_freq'].update(ul_chunk)
                except: pass
            ...
            # Petterson 48k band â†’ higher quality audio for voice/correlation detectors
            ...
            if self.cycle_count % 3 == 0 and len(lpt_chunk) >= 4096:
                ...
                # Physics: MW hits body â†’ carbon square-law â†’ acoustic emission â†’ mic captures it.
            ...
            # Feeds inverted carbon-demodulated audio through amp â†’ loop
            ...
            if self.cycle_count % 5 == 0 and SOUNDDEVICE_AVAILABLE and self._whisper_model:
                try:
                    ...
                    # Decimate 48kHz â†’ 16kHz, take last 2 seconds
            ...
            if self.cycle_count % 3 == 0 and len(ul_chunk) >= 16384:
                try:
                    ...
                    for pk in peaks[:8]:
                        ...
                        # Sidebands appear at fc Â± voice_freq (80-4000 Hz)
                        bw_half = int(4000 / (self.petterson.fs / n))  # Â±4kHz in FFT bins
            ...
            if self.cycle_count % 3 == 0 and len(ul_chunk) >= 16384:
                try:
                    ...
                    for pk in peaks[:10]:  # top 10 carriers
                        ...
                        #   - comes from MWâ†’bodyâ†’carbon square-law interaction
                        ...
                        if bw < 50:
                            # Narrow CW = carbon interaction (MW â†’ body â†’ ultrasound tone)
            ...
            audio_detectors={'eardrum','constant_sonic','ai_voice','sstv','isolation_booth','forced_thought','silent_sound','replay','beat_freq'}
                            ...
                            try:
                                ...
                                if matched_freq > 0:
                                    ...
                                    if audio_demod is not None and len(audio_demod) > 0:
                                        self.log.info(f"ðŸ”Š Demodulated {matched_freq}Hz: {content}")
                                        # Save to evidence
                                    ...
                                    for c2 in c2_results:
                                        self.log.info(f"ðŸ“¡ C2 protocol: {c2['protocol']} on {matched_freq}Hz - {c2.get('note','')}")
                                        self.court._write_chain({
                                            "type": "c2_analysis",
                                            "freq_hz": float(matched_freq),
            ...
            # 9a. Cross-layer parametric attack correlation
            try:
                self.cross_layer.update(sources)
                corr_alerts = self.cross_layer.correlate()
                for alert in corr_alerts:
                    fp = f'parametric_attack_{hash(str(alert.get("specific_threats",[])))%10000}'
                    self.localization.add_observation(
                        fingerprint=fp.encode() if isinstance(fp, str) else fp,
                        obs_lat=lat, obs_lon=lon,
                        bearing_deg=self.aoa if self.aoa != 0.0 else None,
                        range_m=None, freq=0,
                        classification='transmitter',
                        detector_name='parametric_attack_confirmed',
                        snr=float(alert.get('confidence', 0.5) * 50),
                        source_type='active')
                    self.log.warning(
                        f'PARAMETRIC ATTACK: layers={alert["total_layers"]} '
                        f'confidence={alert["confidence"]:.2f} '
                        f'threats={alert.get("specific_threats",[])}')
                # Auto-jammer: intelligent enable/disable based on attack confidence
                try:
                    max_confidence = max([a.get('confidence', 0) for a in corr_alerts]) if corr_alerts else 0.0
                    beat_count = sum(1 for s in sources if s.get('detector') == 'parametric_beat_pair')
                    self.us_jammer.auto_decide(max_confidence, beat_count)
                    # Effectiveness monitoring: compare beat pairs across cycles
                    if hasattr(self.us_jammer, '_prev_beat_count'):
                        self.us_jammer.update_effectiveness(
                            self.us_jammer._prev_beat_count, beat_count)
                    self.us_jammer._prev_beat_count = beat_count
                except: pass
            except Exception as _pc_err:
                if self.cycle_count % 50 == 0:
                    self.log.debug(f'Cross-layer correlator: {_pc_err}')
            ...
            # DECOY SUBTRACTION: cross-sensor coherence check WITH TRIANGULATION
            try:
                coherent_hits = self.decoy_subtraction.check_cross_sensor_coherence()
                if coherent_hits:
                    decoy_s = self.decoy_subtraction.get_stats()
                    self.log.warning('CROSS-SENSOR HIDDEN: %d signals on 2+ sensors | decoys active=%d hidden total=%d' % (
                        len(coherent_hits), decoy_s["active_decoys"], decoy_s["total_hidden_found"]))
                    for ch in coherent_hits:
                        sensors_str = '+'.join(ch['sensors'])
                        self.log.warning('  HIDDEN [coherent]: %.3fMHz SNR=%.1fdB sensors=%s hits=%d' % (
                            ch['freq']/1e6, ch['snr_db'], sensors_str, ch['n_hits']))
                        # TRY TRIANGULATION: add observation from BladeRF position with residue AoA
                        # This gives the localization engine a real bearing for intersection
                        bearing_for_tri = self.aoa if self.aoa != 0.0 else None
                        self.localization.add_observation(
                            fingerprint='hidden_coherent_%d' % int(ch['freq']),
                            obs_lat=lat, obs_lon=lon,
                            bearing_deg=bearing_for_tri,
                            range_m=None, freq=float(ch['freq']),
                            classification='hidden_signal',
                            detector_name='decoy_cross_sensor_coherent',
                            snr=float(ch['snr_db']))
                        # ALSO add from HackRF position (if available) for cross-sensor triangulation
                        if hasattr(self.localization, 'hackrf_lat') and self.localization.hackrf_lat is not None:
                            # Get latest HackRF bearing (stored each cycle from HackRF section)
                            hackrf_bearing = getattr(self, '_last_hackrf_aoa', None)
                            if hackrf_bearing is not None:
                                self.localization.add_observation(
                                    fingerprint='hidden_coherent_hackrf_%d' % int(ch['freq']),
                                    obs_lat=self.localization.hackrf_lat,
                                    obs_lon=self.localization.hackrf_lon,
                                    bearing_deg=float(hackrf_bearing),
                                    range_m=self.hackrf_range, freq=float(ch['freq']),
                                    classification='hidden_signal',
                                    detector_name='decoy_cross_sensor_hackrf',
                                    snr=float(ch['snr_db']))
            except Exception as _cse:
                if self.cycle_count % 50 == 0:
                    self.log.debug('Cross-sensor coherence error: %s' % str(_cse)[:80])
            ...
            # NULL STEERING + AGGRESSIVE JAMMING: Counter ALL transmitter sources
            # Covers MW voice (2.45 GHz), HackRF ferrite (570 MHz), injection signals, etc.
            # Uses time-division multiplexed jamming: cycle through all threats rapidly.
            if self.null_enabled and self.aoa != 0.0:
                try:
                    # Build prioritized jam target list: transmitters sorted by threat
                    _jam_targets = []
                    for s in sources:
                        det = (s.get('detector','') or '').lower()
                        cls = (s.get('classification','') or '').lower()
                        freq = s.get('freq', 0)
                        snr = s.get('snr', 0) or 0
                        bearing = s.get('bearing', self.aoa)
                        # Include ALL transmitters AND high-SNR signals
                        is_target = False
                        priority = 0
                        if freq >= 100e6:
                            if cls == 'transmitter':
                                is_target = True
                                if 'mw_voice' in det or 'carbon' in det:
                                    priority = 100 + snr  # MW voice = highest priority
                                elif 'ferrite' in det or 'hackrf' in det:
                                    priority = 80 + snr   # Direction-finding confirmed
                                elif 'injection' in det or 'ecpri' in det:
                                    priority = 60 + snr
                                else:
                                    priority = 40 + snr
                            # Also jam strong victims (they indicate active attack)
                            elif cls == 'victim' and snr > 20:
                                is_target = True
                                priority = 30 + snr
                        if is_target and freq > 0:
                            _jam_targets.append((priority, freq, bearing, snr, det[:30]))
                    # Sort by priority, jam highest first
                    _jam_targets.sort(key=lambda x: x[0], reverse=True)
                    for _pri, _freq, _brg, _snr, _det in _jam_targets[:4]:  # top 4 threats
                        depth = self.null_steering.update(_freq, _brg, _snr)
                        if depth > 2:
                            _label = f'{_freq/1e6:.0f}MHz' if _freq < 1e9 else f'{_freq/1e9:.2f}GHz'
                            self.log.info(f'JAM: {_label} ({_det}) depth={depth:.1f}dB')
                except Exception as ex:
                    if self.cycle_count % 20 == 0:
                        self.log.debug(f'Null steer error: {ex}')
                # Feed TDM jam target list for frequency-hopping jamming
                if _jam_targets:
                    tdm_list = [(_freq, _pri) for _pri, _freq, _brg, _snr, _det in _jam_targets[:4]]
                    self.null_steering.set_tdm_targets(tdm_list)
            ...
            # Blind spot analysis (every 30 cycles) â€” find coverage gaps
            if self.cycle_count % 30 == 0:
                try:
            ...
            # Modulation fingerprinting â€” classify IQ from HackRF when available
            ...
            # Cycle anomaly detection â€” detect statistical outliers in system metrics
            if self.cycle_count % 5 == 0:
                try:
            ...
            wigle_aps=[]
            aps = []  # initialize before try block â€” prevents NameError if scan fails
            try:
            ...
            # 11a. GPS Jam Scanner â€” check for GPS jamming signals
                    ...
                    if iq1_pre is not None and len(iq1_pre) > 512:
                        ...
                        if iq1_post is not None and len(iq1_post) > 512:
                            ...
                            if iq1_late is not None and len(iq1_late) > 512:
                                ...
                                # Analysis:
                                # Real transmitter: post_power â‰ˆ pre_power (own power source)
            ...
            # 11b. Active direction probe: TX pulse â†’ RX echo â†’ measure return direction
            ...
            if _rnd.random() < 0.05 and self.bladerf_cli is not None and bladerf_active:  # random direction probe
                try:
                    ...
                    if probe_iq1 is not None and len(probe_iq1) > 100:
                        ...
                        self.log.info(f"DIRECTION PROBE: p1={echo_p1} p2={echo_p2} ratio={echo_ratio} delta={delta} â†’ {direction}")
                ...
                try:
                    ...
                    if base_iq1 is not None and len(base_iq1) > 2048:
                        ...
                        if echo_iq1 is not None and len(echo_iq1) > 2048:
                            ...
                            # with echo (ambient + reflections). Envelope lag â†’ delay â†’ range.
            ...
            # 12. Update map
            map_data={
                'observer':{'lat':lat,'lon':lon,'aoa':self.aoa,'aoa_alt':getattr(self,'aoa_alternate',self.aoa+180 if self.aoa!=0 else 0),'gps_fix':gps['has_fix'],'gps_source':gps.get('source','fixed'),
                ...
                'hidden_signals':self.hidden_hunter.get_report(), 'decoy_subtraction':self.decoy_subtraction.get_stats(),
                ...
                'parametric_correlation': self.cross_layer.get_status(clean_sources),
                'us_jammer': {'enabled': self.us_jammer.enabled, 'effectiveness': self.us_jammer.effectiveness},
            ...
            # WIGGLE: Jittered cycle delay â€” adversary cannot predict next capture window
